...
| configuration | storage ([SmartStorage](options/smart-storage.md)), options ([SmartClientOptions](options/client-options.md)), cache ([CacheOptions](options/cache-options.md)) | Options storage, cache and optional features |
...
| clientConfig  | [SmartClientConfiguration](./smart-client-configuration.md)                                                                                                     | Smart on FHIR client configuration                 |
| configuration | storage ([SmartStorage](options/smart-storage.md)), options ([SmartClientOptions](options/client-options.md)), cache ([CacheOptions](options/cache-options.md)) | Options for enabling or disabling certain features |
...
the URL by the application and stored in for example
[sessionStorage](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage).

This ID _must_ be used in any subsequent instantiations of the SmartClient (and it's subsequent
`.ready()` calls) to ensure that each session is correctly associated with the launched patient.

An example of the typical flow when using multi-launch mode, steps 1-5 are normal, 6-8 are specific
to multi-launch mode:
...
7. The web-application retrieves the query parameter, stores it in `sessionStorage`, and removes it
   from the URL.
8. The web-application passes the patient ID (now in session storage) back to the server when the
   server needs to fetch any data, and the SmartClient is instantiated with both the sessionId
   (cookie) and activePatient (from session storage, passed to the server) and creates a
   `ReadyClient` scoped to that specific patient.
